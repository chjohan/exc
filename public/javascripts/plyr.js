    ...
    function Plyr(container, config) {
        ...
        function _setupMedia() {
            ...
            if (plyr.supported.full) {
                ...
                // Inject the player wrapper and video-intro wrapper
                if (plyr.type === 'video') {
                    // Create the wrapper div
                    var wrapper = document.createElement('div');
                    wrapper.setAttribute('class', config.classes.videoWrapper);

                    var videoIntroWrapper = document.createElement('div');
                    videoIntroWrapper.setAttribute('class', config.classes.introWrapper);

                    _wrap(plyr.media, videoIntroWrapper);
        ...
        // Get next video, plays it and updates UI.
        function _playNextVideo() {
            var getUrl = location.pathname+'/video';
            
            $.getJSON(getUrl, function(data) {
                // Update interface
                $('#video-info h1').text(data.title);
                $('#video-info p').text(data.description);

                // Extract description
                //$('.js-plyr').html('<div class="hello"></div>');
                
                // Remove children
                $('.plyr__video-wrapper').css("visibility", "hidden");
 
                // Set overlay div
                var newDiv = $('<div id="videoIntro"><div id="coming-up-container"></div><div id="text"><h1>Hello</h1></div></div>');
                $('.plyr').prepend(newDiv);

                var nextUp = "<span class='next-up'>Next up</span>",
                stS = "<span class='slabtext'>",
                stE = "</span>";
                
                var textEle = $('#videoIntro #text');
                var comingUpEle = $('#videoIntro #coming-up-container');
                comingUpEle.html('<h3>Coming up</h3>');
                textEle.css('visibility', 'hidden');
                textEle.html(stS + data.slabTitle.join(stE + stS) + stE).slabText();
                textEle.hide();

                delete data.description;
                delete data.category;
                delete data.slabTitle;

                var secCounter = -1,
                    secDuration = 9;
                var check = function() {
                    secCounter++;
                    if (secCounter == secDuration) {
                        // Remove overlay with title
                        
                        
                        // Remove children
                        $('.plyr #videoIntro').remove(); 
                        $('.plyr__video-wrapper').css("visibility", "visible");
                        // Update player with new source
                        _updateSource(data);
                    } else {
                        if (secCounter == 2) {
